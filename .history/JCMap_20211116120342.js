...
import img0 from './assets/images/map/m0.png'
import img1 from './assets/images/map/m1.png'
import img2 from './assets/images/map/m2.png'
import img3 from './assets/images/map/m3.png'
function initJCMap() {
	...
	function createOverlayMarker(id, content, position) {
	...
	function TemporaryMap(target = 'map', options) {
		const { center = [0, 0], zoom = 6, minZoom = 0, maxZoom = 28, doubleClickZoom = false } = options
		// 标准底图图层
	...
	function _JCMap(...args) {
		...
		this.addOverlay = map.addOverlay.bind(map) // 添加覆盖物
		...
		this.setAllMarker = features => {
			const commonStyle = option => {
				...
				return new Style({
					...
					text: new Text({
						...
						offsetY: singleStyle.offsetY,
						offsetX: singleStyle.offsetX,
						padding: singleStyle.padding,
						backgroundFill: new Fill({
							color: singleStyle.backgroundFill
						}),
			...
			const pointLayer = new VectorLayer({
				source: source,
				style: feature => commonStyle(feature)
			})
	...
	function _JCMarker(option) {
		...
		const overlayMarker = createOverlayMarker('', content, position)
	...
	function _MarkerClusterer(map, options, features = []) {
		...
		function createMarkerClusterer(map, options, features) {
			...
			vectorSource.forEachFeature(marker => {
				!marker.options.content && marker.setStyle(createMarkerStyleFn(marker.options.style))
			})
					...
					if (features.length == 1) {
						setTimeout(() => {
							const marker = createOverlayMarker(
								features[0].getProperties().extData.id,
						...
						if (target instanceof _JCMap) {
							if (target.hasFeatureAtPixel(e.pixel)) {
								...
								if (typeName === 'Point') {
									...
									console.log(target.getFeaturesAtPixel(e.pixel)[0].getGeometry().getType())
									if (features.length == 1) {
										// 单个要素
