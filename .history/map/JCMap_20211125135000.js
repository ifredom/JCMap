...
const standardTileLayerUrl = 'http://t0.tianditu.gov.cn/DataServer?T=img_c&x=0&y=0&l=1&tk=9ce65cda6a7cad4da8b250601e8c2802'
const satelliteTileLayerUrl = 'http://t0.tianditu.gov.cn/DataServer?T=img_w&x=0&y=0&l=1&tk=9ce65cda6a7cad4da8b250601e8c2802'
...
function getLayerUrlByData(type, wkid, token) {
	...
	————————————————
	版权声明：本文为CSDN博主「王巧_1」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
	原文链接：https://blog.csdn.net/u014380802/article/details/119766216
	...
	return new OlMap({
		target,
		layers: [satelliteTileLayer, standardTileLayer, vectorLayer], // 图层
		overlays: [], // 覆盖物
		view: new OlView({
	...
	this.removeMarker = (...args) => {
		if (args.length === 1) {
			...
			if (Array.isArray(args[0])) {
				...
				map.removeOverlay(args[0].getOverlayMarker())
			}
		} else {
			// 多个参数
			args.forEach(marker => {
