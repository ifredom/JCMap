...
function initJCMap(target = 'map', options = {}) {
	...
	//创建天地图矢量底图
	const layerTianDiTu = new TileLayer({
		zIndex: 0, // 底图图层层级
		visible: true, //底图显示隐藏
		className: 'ol-layer standard',
		source: new XYZ({
			url:
				'http://t{0-7}.tianditu.gov.cn/img_w/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&STYLE=default&TILEMATRIXSET=w&FORMAT=tiles&TILEMATRIX={z}&TILEROW={x}&TILECOL={y}&tk=' +
				token,
	...
	return new OlMap({
		target,
		layers: [satelliteTileLayer, standardTileLayer, layerTianDi, layerTianDiLabel, layerTianDiImg, layerTianDiImgLabel, layerTianDiTu, vectorLayer], // 图层
		overlays: [], // 覆盖物
		view: new OlView({
