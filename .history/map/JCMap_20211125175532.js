...
function JCMap(...args) {
	...
	this.addLayer = layer => {
		...
		if (layer.getClassName().indexOf('jc-clusterer-layer') !== -1) {
			this.markerCluster = layer
		}
		map.addLayer(layer)
	} // 添加图层
	...
	this.removeMarker = (...args) => {
		if (args.length === 1) {
			...
			if (Array.isArray(target)) {
				...
				// 此处只处理了聚合物的 OverlayMarker
				this.markerCluster.forEachFeature(() => {
					this.markerCluster.removeFeature(target)
				})
			} else if (target.JCName === 'OverlayMarker') {
