 ...
 * @returns
 */
function createMarkerClusterer(map, options, markers) {
	const { distance = 40, minDistance = 30 } = options || {}
	const styleCache = {}
	const features = markers.map(marker => marker[marker.JCTYPE])
	...
	const clusterSource = new OlCluster({
		...
		createCluster(point, features) {
			if (features.length == 1) {
				...
				return overlayMarker ? overlayMarkerCluster : markerCluster
			} else {
	...
	clusterSource.on('change', function (e) {
		...
		clusterSource.overlaysList.forEach(item => {
			const overlay = map.getOverlayById(item)
