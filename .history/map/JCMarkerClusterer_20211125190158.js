	...
	this.addMarker = (...args) => {
		if (args.length === 1) {
			if (Array.isArray(args[0])) {
				const markers = args[0].filter(marker => marker.JCName === 'OVERLAYMARKER')
				this.getVectorSource().addFeatures(markers)
			} else {
				args[0].JCName === 'OVERLAYMARKER' && this.getVectorSource().addFeature(args[0].OVERLAYMARKER)
			}
		} else {
		...
		if (args.length === 1) {
			...
			if (Array.isArray(args[0])) {
				args[0].forEach(marker => {
					this.removeOverlayMarker(marker.getOverlayMarker())
					args[0].JCName === 'OVERLAYMARKER' && this.getVectorSource().removeFeature(marker) // 删除Feature Marker
				})
			} else if (args[0].JCName === 'OVERLAYMARKER') {
				// removeFeature 时候 触发 change 事件，并overlayMarker
				// map.removeMarker(args[0])
