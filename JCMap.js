...
function initJCMap() {
  ...
  function _JCMap(...args) {
    ...
    this.removePaint = (feature) => {
      ...
      if (vectorGraph) {
        if (
          Object.prototype.toString.call(feature) === '[object Number]' ||
          Object.prototype.toString.call(feature) === '[object String]'
        ) {
  ...
  function _MarkerClusterer(map, options, features = []) {
    const clusterLayer = createMarkerClusterer.call(
      this,
      map,
      options,
      features
    ...
    function createMarkerClusterer(map, options, features) {
        ...
        features.forEach((feature) => {
          if (!feature.get('features')) {
            ...
            if (marker) {
              ...
              const inViewGeometry =
                viewGeometry.intersectsCoordinate(coordinate) // 是否在可视区域内
              if (!clusterSource.overlayIds.includes(id) && inViewGeometry) {
      ...
      clusterSource.on('change', function (e) {
        ...
        clusterSource.overlayIds.forEach((item) => {
      ...
      const clusterLayer = new VectorLayer({
        ...
        style: (feature, resolution) =>
          createFeatureStyleFn(styleCache, feature, resolution),
      })
      return clusterLayer
    }
    ...
    this.setDistance = (distance) =>
      this.clusterLayer.getSource().setDistance(distance)

    this.getMinDistance = () => this.clusterLayer.getSource().getMinDistance() // 聚合物的最小间距

    this.setMinDistance = (minDistance) =>
      this.clusterLayer.getSource().setMinDistance(minDistance)
    ...
    this.setMarkers = (features) => {
      ...
      if (Array.isArray(features) && features.length > 0) {
        ...
        this.clusterLayer = createMarkerClusterer(
          map,
    ...
    this.addMarker = (...args) => {
      if (args.length === 1) {
        if (Array.isArray(args[0])) {
          ...
          args[0] instanceof Marker &&
            this.getVectorSource().addFeature(args[0])
        }
      } else {
    ...
    this.removeOverlayMarker = (marker) => {
      const overlayMarker =
        marker instanceof Overlay ? marker : marker.get('overlayMarker')
      if (overlayMarker) {
        ...
        if (Array.isArray(args[0])) {
          args[0].forEach((marker) => {
            ...
            marker instanceof Marker &&
              this.getVectorSource().removeFeature(marker) // 删除Feature Marker
          })
        } else {
          this.removeOverlayMarker(args[0])
          args[0] instanceof Marker &&
            this.getVectorSource().removeFeature(args[0])
        }
      } else {
        // 多个参数
        args.forEach((marker) => {
    ...
    this.clearMarkers = () => {
      ...
      this.getClusterSource().overlayIds.forEach((item) => {
        const over = map.getOverlayById(item)
        map.removeOverlays(over)
      })
  ...
  function _JCGraph(map, options = {}) {
    ...
    this.activate = function (graphName) {
      if (graphTool[graphName]) {
        ...
        console.warn(
          `JCMap vectorGraph has not ${graphName}, please you sure and try again~`
        )
      }
    }
    /**
    ...
    this.editPaint = () => {
      this.status = false
      const defaultStyle = new Modify({ source: this.source })
        .getOverlay()
        .getStyleFunction()
                ...
                if (currentRadius > 0) {
                  ...
                  if (typeName !== 'Circle') {
                    ...
                    if (
                      !(
                        typeName === 'Polygon' &&
                        compareArray(tmp[0], tmp[tmp.length - 1])
                      )
                    ) {
            ...
            timeoutID = setTimeout(() => {
              ...
              if (target instanceof _JCMap) {
                console.log(
                  '当前坐标',
                  transform(
                    target.getCoordinateFromPixel(e.pixel),
                    target.getView().getProjection(),
                    'EPSG:4326'
                  )
                )
                if (target.hasFeatureAtPixel(e.pixel)) {
                  ...
                  if (typeName === 'Point') {
                    ...
                    if (features) {
                      if (features.length > 1) {
                        // 聚合物
                        const clusterFeatures = target.getFeaturesAtPixel(
                          e.pixel
                        )[0]
                    ...
                    result = [
                      target.getFeaturesAtPixel(e.pixel)[0].getGeometry(),
                      typeName,
                    ]
          ...
          if (target instanceof _JCMap) {
            ...
            target.on('pointermove', (e) => {
              ...
              timeoutHoverID = setTimeout(() => {
                ...
                if (target.hasFeatureAtPixel(e.pixel)) {
                  const feature = target.forEachFeatureAtPixel(
                    e.pixel,
                    function (feature) {
                      return feature
                    }
                ...
                const geometryType =
                  geometryCache && geometryCache.getGeometry().getType()
