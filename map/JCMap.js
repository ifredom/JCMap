...
function getLayerUrlByData(type, wkid, token) {
  ...
  return (
    url + "T=" + layerId + tileMatrixSetId + "&x={x}&y={y}&l={z}&tk=" + token
  );
}
...
function JCMap(target = "map", options = {}) {
  ...
  this.on = (eventName, callBack = () => {}, option = {}) => {
    ...
    if (
      ...
      eventName.substring(0, 8) !== "JCMarker" &&
      eventName.substring(0, 9) !== "JCCluster"
    )
      ...
      const eventHandler = (e, olMap, eventObject, callBack) => {
        ...
        clickTimeId = setTimeout(() => {
          if (olMap.hasFeatureAtPixel(e.pixel)) {
            // 获取点击的图层，以及要素
            const [olFeature, olLayer] = olMap.forEachFeatureAtPixel(
              e.pixel,
  ...
  this.off = (eventName, callBack = () => {}) => {
    ...
    if (index !== -1) {
      ...
      map.un(
        isJCMarkerEvent ? mapEventName[1] : currentEventObject.eventName,
  ...
  this.setFitView = () => {
    ...
    if (
      clusterLayer &&
      clusterLayer.getSource().getSource().getFeatures().length
    ) {
