...
function JCMap(target = 'map', options = {}) {
	...
	// 事件监听
	this.on = (eventName, callBack = () => {}, clickTimeDelay = 200) => {
					...
					if (olMap.hasFeatureAtPixel(e.pixel)) {
						...
						const dispatchAction = (target, eventTarget, JCEvents, event) => {
							JCEvents.forEach(function (currentEventObject, eventName) {
								if (eventName.indexOf(getUid(target)) !== -1 && /\((.+)\)/.exec(eventName)[1] === e.type) {
	...
	// 默认的矢量图层
	this.vectorLayer = new JCVectorLayer({ map: this, className: 'jc-vector-layer' })
}
