...
function createOverlayMarker(options) {
  ...
  const element = createOverlayMarkerElement(
    content,
    id,
    options.style.overlayMarker
  );
  ...
  if (this.JCTYPE === "MARKER") {
    ...
    this.on = (eventName, callBack) => {
        ...
        switch (eventName) {
          ...
          case "dblclick":
            eventObject.handler = (e) =>
              dblclickHandler(eventName, e, callBack);
            break;
          case "contextmenu":
            eventObject.handler = (e) =>
              contextmenuHandler(eventName, e, callBack);
            break;
          default:
            break;
        }
      ...
      if (!this.overlayMarkerEvents.has(eventObject.target)) {
        ...
        //绑定过事件
        const currentEventArray = this.overlayMarkerEvents.get(
          eventObject.target
        );
        ...
        if (!currentEventObject) {
          ...
          //绑定过此事件，移除之前事件绑定再覆盖
          eventObject.target.removeEventListener(
            eventName,
            currentEventObject.handler
          );
          this.overlayMarkerEvents.set(eventObject.target, [eventObject]); //保存监听事件并执行
